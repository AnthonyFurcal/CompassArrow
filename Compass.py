...
def compass_overlay(image):
   ...
   if overlay.shape[2] == 4:  # Color image with alpha channel
       b, g, r, alpha = cv2.split(overlay)
       alpha = alpha / 255.0  # Normalize alpha to be between 0 and 1
       overlay_rgb = cv2.merge([b, g, r])
   elif len(overlay.shape) == 2:  # Grayscale image
        alpha = overlay / 255.0
        overlay_rgb = overlay
   ...
   blended_roi = (roi_rgb * (1 - resized_alpha[:, :, np.newaxis]) + overlay_rgb * resized_alpha[:, :, np.newaxis]).astype(np.uint8)
